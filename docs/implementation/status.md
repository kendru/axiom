...
| Code generation | WASM backend compiles integer and boolean literals, arithmetic primops (`add`, `sub`, `mul`, `neg`), comparison operators (`lt`, `gt`, `lte`, `gte`, `eq`), let-bindings, function definitions and calls, algebraic data types (constructor allocation and pattern matching including tag dispatch, field extraction, or-patterns, and wildcards), parameterised ADT types (`TyApp` lowered to i32), string literals and basic string operations (`concat`, `string_length`, `string_eq`, `string_head`), and a built-in `max` helper. The ctor_map automatically registers both the canonical `Ctor` and lowercase `ctor` forms so stdlib-style calls (e.g. `none()`, `cons(h,t)`) compile transparently. See [docs/implementation/codegen.md](docs/implementation/codegen.md) | Functions with `TyFun` parameter types (higher-order functions such as `map`, `filter`, `fold_left`) are silently skipped. Records, floating-point arithmetic, and `TyTuple` params are not yet implemented. No runtime system or standard library support. |
