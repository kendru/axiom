...
| **Code generation** | §9 | Partial WASM backend in `lib/codegen.ml` and `lib/wasm/wasm_encode.ml`; integers, booleans, conditionals, let/letrec, function calls, and simple pattern matching work; records, effects, strings, and several expression forms are not yet handled. |
